...
export default function TopNav() {
  const { view, setView, machines, statuses, loadAll, zoom, setZoom } = useStore()
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!res.ok) {
      alert(`Erreur lors de l'import : ${res.status} ${res.statusText}`)
      return
    }
    await loadAll()
  }
  ...
  return (
    <div className="flex items-center h-11 bg-gray-800 border-b border-gray-700 px-4 gap-3 flex-shrink-0">
      ...
      <div className="flex gap-1">
        {(['inventory', 'plans'] as const).map(v => (
          <button key={v} onClick={() => setView(v)}
            className={`px-3 py-1 rounded text-xs font-medium transition-colors ${view === v ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}>
      ...
      {machines.length > 0 && (
        <>
          ...
          <div className="flex items-center gap-2 text-xs">
            <span title={`${online} en ligne`} className="flex items-center gap-1 text-green-400">
              <span style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: '#22c55e', display: 'inline-block' }} />
              {online}
            </span>
            <span title={`${offline} hors ligne`} className="flex items-center gap-1 text-red-400">
              <span style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: '#ef4444', display: 'inline-block' }} />
              {offline}
            </span>
            {unknown > 0 && (
              <span title={`${unknown} inconnu`} className="flex items-center gap-1 text-gray-500">
      ...
      <div className="ml-auto flex items-center gap-1">
        <button onClick={handleExport} className="px-2 py-1 rounded text-xs text-gray-300 hover:bg-gray-700" title="Exporter en JSON">⬇ Export</button>
        <button onClick={() => importRef.current?.click()} className="px-2 py-1 rounded text-xs text-gray-300 hover:bg-gray-700" title="Importer depuis JSON">⬆ Import</button>
